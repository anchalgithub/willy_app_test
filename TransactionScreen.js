...
//          //ToastAndroid.show(transactionMessage, ToastAndroid.SHORT);//C72
...
// for Isse if the bookAvailability is true
initiateBookIssue = async ()=>{ 
    //add a transaction
    db.collection("transactions").add({
      'studentId' : this.state.scannedStudentId,
...
// for Return if the bookAvailability is false
  initiateBookReturn = async ()=>{
    //add a transaction
    db.collection("transactions").add({
      'studentId' : this.state.scannedStudentId,
  ...
  handleTransaction = async () => {
    ...
    if (!transactionType) {
      ...
      var isStudentREligible = await this.checkStudentEligibilityForReturn();
...
checkBookEligibility = async () => {
  ...
  handleBarCodeScanned = async ({ type, data }) => {
    ...
    if (buttonState === 'BookId') {
      this.setState({
        scanned: true,
        scannedBookId: "bk0001",
        buttonState: 'normal',
      });
    } else if (buttonState === 'StudentId') {
      this.setState({
        scanned: true,
        scannedStudentId: "ST001",
        buttonState: 'normal',
      });
    }
  };
        ...
        <View style={styles.container}>
          ...
          <View style={styles.inputView}>
            <TextInput
              style={styles.inputBox}
              placeholder="Book Id"
              onChangeText ={text => this.setState({scannedBookId:text})} //C72
              value={this.state.scannedBookId}
            />
            <TouchableOpacity
              style={styles.scanButton}
          ...
          <View style={styles.inputView}>
            <TextInput
              style={styles.inputBox}
              placeholder="Student Id"
              onChangeText ={text => this.setState({scannedStudentId:text})}//C72
              value={this.state.scannedStudentId}
            />
            <TouchableOpacity
              style={styles.scanButton}
